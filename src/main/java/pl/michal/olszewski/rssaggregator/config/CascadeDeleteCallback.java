...
public class CascadeDeleteCallback implements ReflectionUtils.FieldCallback {
  ...
  CascadeDeleteCallback(final Object source, final MongoOperations mongoOperations) {
    this.source = source;
    ...
    if (field.isAnnotationPresent(DBRef.class) && field.isAnnotationPresent(CascadeSave.class)) {
      ...
      if (fieldValue != null) {
        ...
        if (Collection.class.isAssignableFrom(field.getType())) {
          final Collection<Object> fieldValues = (Collection<Object>) field.get(getSource());
          for (Object value : fieldValues) {
            getMongoOperations().remove(value);
          }
        } else {
